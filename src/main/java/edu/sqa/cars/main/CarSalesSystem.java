...
public class CarSalesSystem extends JFrame implements ActionListener, ComponentListener, ChangeListener {
  ...
  public CarSalesSystem(String f) {
    ...
    try {
      ...
      logger.log(WARNING, "The data file, \"cars.dat\" does not exist. Please create an empty file named \"cars.dat\"");
      System.exit(0);
    }
    // empty cars.dat file, this error should be ignored
    catch (java.io.IOException exp) {
      logger.log(WARNING, "The data file, \"cars.dat\" is possibly corrupted. Please delete it and create a new empty data file named \"cars.dat\"");
      System.exit(0);
    } catch (Exception exp) {
      logger.log(WARNING, "There was an error loading \"cars.dat\". Try deleting and creating a new empty file named \"cars.dat\"");
      System.exit(0);
    }
  ...
  public void setCarsUpdated() {
    ...
    for (Object registeredListener : registeredListeners) {
      ...
      try {
        ...
        logger.log(WARNING,
            "'public carsUpdated(CarEvent)' method does not exist in {}. You will not receive any car update events",
            registeredListener.getClass().getName());
      } catch (IllegalAccessException exp) {
        logger.log(WARNING,
            "the 'public carUpdated(CarEvent)' method could not be called for unknown reasons, You will not receive any car update events");
      } catch (InvocationTargetException e) {
        logger.log(WARNING,
            "the 'public invoke(Object)' method could not be called for unknown reasons, You will not register carsUpdated event");
      }
    }
  }
